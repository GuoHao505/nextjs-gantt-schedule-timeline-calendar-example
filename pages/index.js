...
async function initializeGSTC(element) {
  ...
  const HighlightWeekends = (
    await import(
      "gantt-schedule-timeline-calendar/dist/plugins/highlight-weekends.esm.min.js"
    )
  ).Plugin;
  ...
  function onLevelDates({ dates, level, format, time }) {
    ...
    return dates.filter(
      (date) =>
        date.leftGlobalDate.day() !== 0 && date.leftGlobalDate.day() !== 6
    );
  }
  /**
...
export default function Home() {
  ...
  return (
    <div className="container">
      ...
      <hr />
      <label>
        <input type="checkbox" onChange={handWeekends} />
        Hide Weekends
      </label>
